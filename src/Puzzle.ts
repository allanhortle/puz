...
type PuzzleData = {
  ...
  rawBoard: {black: boolean; number?: number, circle?: boolean}[][];
...
export default class Puzzle {
  ...
  static async fromFile(file: string) {
    ...
    while (stringData.length > 1) {
      ...
      if (toString(data).match(/(GEXT)|(GRBS)|(RTBL)/)) {
        ...
        console.log({width});
        const content = stringData.subarray(0, width * height);
        if (title === 'RTBL') {
        ...
        if (title === 'GEXT') {
          content.forEach((ii, index) => {
            const column = ((index + 1) % width);
            const row = Math.floor(index / width);
            if (ii > 0) {
              console.log({spot: board[row], index, column, row});
            }
    ...
    const answerUrl = `https://www.xwordinfo.com/Crossword?date=${d.getMonth() + 1
  ...
  printSolution() {
    const {rawBoard} = this.data;

    const output = this.data.solution.map((row, rowIndex) => {
      return row.split('').map((col, colIndex) => {
        const data = rawBoard[rowIndex][colIndex];
        if ('circle' in data) {
          if (data.circle) return chalk.green(col);
